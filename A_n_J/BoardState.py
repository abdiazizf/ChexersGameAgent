...
class BoardState(object):
    ...
    def evaluation_function(self,colour):
        ...
        a = self.safe_pieces(colour)
        return 10*(material_weight/opposing_material) + (self_exits/10*opposing_exits) + self.wins(colour) + 100 * a

    def safe_pieces(self, colour):
        safe_pieces = []
        all_pieces = chain.from_iterable(self.piece_vectors)
        opponent_pieces = diff(all_pieces, self.piece_vectors[colour])
        for my_piece in self.piece_vectors[colour]:
            ...
            for opponent_piece in opponent_pieces:
                ...
                if direction not in axial_directions or jump_move not in valid_board or jump_move in all_pieces:
                    continue
                safe = False
            if safe:
                safe_pieces.append(my_piece)

        return len(safe_pieces)
